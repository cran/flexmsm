# Note: this function is an adaptation of the vis.gam() function from the R package mgcv

vis.gam.msm  = function (x, view = NULL, cond = list(), n.grid = 30, too.far = 0,
                          ...
                          s1 = s1, s2 = s2, ...)
{
  ...
  if (se <= 0) {
    ...
    if (!is.null(zlim)) {
      ...
      max.z <- max(exp(fv$fit + fv$se.fit * se), na.rm = TRUE) # HERE
      min.z <- min(exp(fv$fit - fv$se.fit * se), na.rm = TRUE) # HERE
      zlim <- c(min.z, max.z)
    }
    z <- exp(fv$fit - fv$se.fit * se) # HERE
    z <- matrix(z, n.grid, n.grid)
